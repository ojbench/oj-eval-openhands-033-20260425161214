...
public:
    ...
    list() : list_size(0) {
        // Create dummy head and tail nodes using placement new
        // We need to construct them without calling T's constructor
        head = static_cast<node*>(::operator new(sizeof(node)));
        tail = static_cast<node*>(::operator new(sizeof(node)));
        
        // Initialize the pointers manually
        head->next = tail;
        head->prev = nullptr;
        tail->prev = head;
        tail->next = nullptr;
    ...
    virtual ~list() {
        clear();
        // Deallocate dummy nodes without calling destructor
